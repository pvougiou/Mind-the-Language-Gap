...
df = pandas.read_csv('processed/ar-quality.csv')
...
with open('processed/ar-result-quality.csv', 'w') as outfile:
    outfile.write('news_mean\t' + 'wiki_mean\t' + 'ai_mean\t' + '\n')
    outfile.write(str(mean_news) + '\t' + str(mean_wiki) + '\t' + str(mean_ai))

with open('processed/ar-fluency-keys.csv') as inf:
    with open('../../numbers/ar-fluency-quality.txt', 'w') as out:
        news = []
        wiki = []
        ai = []
        for line in inf:
